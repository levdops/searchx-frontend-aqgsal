...
class PreTest extends React.Component {
    ...
    onComplete(data) {
        ...
        var pre_score = 0;
        for (let k in data){
            if (k[0] == 'Q' && k.split("-").length > 2){
                pre_score += parseInt(data[k]);
            }
        }
        console.log("pre-score: ", pre_score);
        if (pre_score >  20  ){
            this.onLeave();
        ...
        SessionStore.updateTask(constants.taskId, data, (res) => {
			...
			res.taskData.qid = 2;
                    AccountStore.setTask(data.taskId, res.taskData);
                    localStorage.setItem("task-qid", res.taskData.qid);
